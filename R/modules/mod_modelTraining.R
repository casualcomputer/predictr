...
mod_modelTraining_server <- function(id, preprocessed_data) {
  moduleServer(id, function(input, output, session) {
    ...
    # Update variable classification UI based on current preprocessed training data
    # This will trigger whenever the preprocessed_data changes, including when a new file is uploaded
    observe({
      ...
      # Get numeric and factor variable indices
      numeric_vars <- vars[sapply(df, is.numeric)]
      factor_vars <- vars[sapply(df, is.factor)]

      # Default outcome
      default_outcome <- if ("picked_up_any" %in% vars) "picked_up_any" else vars[1]

      # Update the UI inputs
      updateSelectInput(session, "var_outcome",
        choices = vars,
        selected = default_outcome
      )

      updateCheckboxGroupInput(session, "var_cont",
        choices = vars,
        selected = numeric_vars
      )

      updateCheckboxGroupInput(session, "var_cat",
        choices = vars,
        selected = factor_vars
      )

      updateCheckboxGroupInput(session, "var_id",
        choices = vars,
        selected = character(0)
      )

      updateCheckboxGroupInput(session, "var_exclude",
        choices = vars,
        selected = character(0)
      )
    })
    ...
    observeEvent(input$train_btn, {
      ...
      # Ensure outcome is properly formatted
      unique_vals <- unique(model_df_train$Outcome)
      if (length(unique_vals) != 2) {
        showNotification("Outcome variable must be binary. Please select a different variable.", type = "error")
        return(NULL)
      }

      if (!all(c("No", "Yes") %in% levels(model_df_train$Outcome))) {
        # For numeric outcomes or other factor levels
        if (is.numeric(model_df_train$Outcome) || !is.factor(model_df_train$Outcome)) {
          model_df_train$Outcome <- factor(ifelse(model_df_train$Outcome == unique_vals[1], "No", "Yes"), levels = c("No", "Yes"))
          model_df_test$Outcome <- factor(ifelse(model_df_test$Outcome == unique_vals[1], "No", "Yes"), levels = c("No", "Yes"))
        } else {
          # For factors with wrong levels
          model_df_train$Outcome <- factor(model_df_train$Outcome, levels = c("No", "Yes"))
          model_df_test$Outcome <- factor(model_df_test$Outcome, levels = c("No", "Yes"))
        }
      }
      ...
      # Ensure factor levels match between train and test for all factors
      for (col in names(train_data_imp)) {
        if (is.factor(train_data_imp[[col]])) {
          test_data_imp[[col]] <- factor(test_data_imp[[col]], levels = levels(train_data_imp[[col]]))
        }
